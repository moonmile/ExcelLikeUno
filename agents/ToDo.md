# ToDo

- [ ] 仮リリースのための README.md の整備
- [x] samples/ に動作方法と実行画面を追加
- [ ] ExcelLikeUno パッケージの利用方法の解説
    - [x] 外部から接続する方法
    - [x] LibreOffice 内部から利用する方法
    - [ ] Linux で利用する方法
    - [ ] WSL で利用する方法
    - [ ] Docker コンテナで利用する方法

## 実装予定

- [x] Cell/Range/Shape.font プロパティの実装
- [x] Cell/Range.borders プロパティの実装
- [x] Shape.line プロパティの実装
- [ ] Shape.margins プロパティの実装
- [x] Shape.fill プロパティの実装
- [x] TextProperties, CharacterProperties の混在を解消
- [x] Chart の実装
- [ ] PivodTable の実装
- [ ] DarwPage の実装


- [ ] DispatchHelper まわりの実装
    - [ ] Calc の保存/読み込みのサポート
    - [ ] Sheet.copy/paste の実装
    - [ ] Cell/Range.copy/paste の実装
    - [ ] Shape.copy/paste の実装
    - [ ] Selection/Select の実装