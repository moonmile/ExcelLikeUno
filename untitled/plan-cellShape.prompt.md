...
**Steps**
1. 現状棚卸し  
   - [src/excellikeuno/table/cell.py](src/excellikeuno/table/cell.py) と [src/excellikeuno/table/cell2.py](src/excellikeuno/table/cell2.py) の責務・差分を整理（value/formula/text、borders/font、CellProperties/CharacterProperties 委譲）。  
...
5. Cell2 の整理  
   - Cell 非継承のまま、Cell と同等の border/font/value/formula API を持たせるか、Cell と共通ヘルパーをモジュール内共有。  
   - テスト `test_cell2.py/test_cell2_border.py` が要求する surface を網羅する。

6. 実装・整合性  
   - 重複ヘルパー（border 正規化など）を１カ所に集約し、Cell/Cell2/Range で共有。  
